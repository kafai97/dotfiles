...
_linode_cli() {
  ...
  linode-cli)
    ...
    ;;
  account)
    COMPREPLY=($(compgen -W "view update cancel invoices-list invoice-view invoice-items logins-list login-view maintenance-list notifications-list clients-list client-create client-view client-update client-delete client-reset-secret payments-list payment-create payment-view paypal-start paypal-execute promo-add settings settings-update enable-managed transfer --help" -- ${cur}))
    return 0
    ;;
  default)
